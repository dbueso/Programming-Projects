...
{                                                                                                       // This is because you must iterate through a list of n elements
    ...
    while (node != &dll.trailer) {                                                                      // This while loop performs n iterations -> O(n) time
        insertLast(node->getElem());                                                                    // Elements from another list are inserted to the new list -> O(1)
...
{                                                                                                       // This is because you must iterate through a list of n elements from a temp list
    ...
    while (node != &temporary.trailer) {                                                                // This while loop performs n iterations -> O(n) time
        insertLast(node->getElem());                                                                    // Elements from temp list are inserted to the new list -> O(1)
...
// insert the object to the first of the linked list                    // F(n) = 6n + 8 -> O(n)
template <typename T>
void TemplateDoublyLinkedList<T>::insert(const T& newobj)               // For n input reocrds this function runs -> O(n)
{
    DListNode<T>* newNode = new DListNode<T>(newobj);                   // 2 operations
    DListNode<T>* iterNode = getFirst();                                // 2 operations

    while (iterNode != getAfterLast()) {                                // Loop through indexed list of nodes -> O(n)
        if (newNode->getElem()((iterNode->getElem()))) {                // Break if "this" node is less than another -> Record &r1(Record &r2)
            newNode->prev = iterNode->prev;                             // Re-assign pointers -> O(1)        
            iterNode->prev->next = newNode;
            newNode->next = iterNode;                                   
            iterNode->prev = newNode;                                   
            break;                                                      // Break out of loop
        }
        iterNode = iterNode->next;                                      // Move the iterating node forward -> O(1)
    }
    if (iterNode == getAfterLast()) {                                   // Node was not less than another node,     2 operations
        insertLast(newNode->getElem());                                 // Insert at the end of this list index,    2 operations
    }
}
...
ostream& operator<<(ostream& out, TemplateDoublyLinkedList<T>& dll) {                   // The running time of this is O(n)
    ...
    while (node != dll.getAfterLast()) {                                                // A while loop is implemented with a "iterator node" -> O(n) time
